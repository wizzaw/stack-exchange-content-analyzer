package com.nayki.api.resources;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.nayki.api.exceptions.DeleteFromCacheException;
import com.nayki.api.exceptions.NotFoundInCacheException;
import com.nayki.api.exceptions.SetInCacheException;
...
import com.nayki.api.services.AnalysisService;
import com.nayki.api.services.DetailService;
import com.nayki.api.services.SummaryService;
import com.nayki.api.utils.SerializationUtils;
import com.nayki.api.utils.StringUtils;
import com.nayki.api.utils.URLUtils;
...
public class AnalysisResource {
    ...
    public Response create(AnalysisRequestBody body) {
        ...
        try {
            ...
            return buildResponse(Response.Status.ACCEPTED, new SerializationUtils().toJson(new AnalysisResponseBody(id)));

        } catch (JsonProcessingException e) {
            return buildResponse(Response.Status.INTERNAL_SERVER_ERROR, "{\"message\": \"Creation of the analysis was failed.\"}");
        } catch (NotFoundException| SetInCacheException | DeleteFromCacheException e) {
    ...
    public Response getSummary(@PathParam("id") String id) {
        ...
        try {
            ...
            response = buildResponse(Response.Status.ACCEPTED, new SerializationUtils().toJson(summary));
        } catch (JsonProcessingException e) {
    ...
    public Response getDetail(@PathParam("id") String id) {
        ...
        try {
            final Analysis analysis = analysisService.get(id);
            final AnalysisDetail detail = detailService.get(analysis.getDetailId());
            response = buildResponse(Response.Status.ACCEPTED, new SerializationUtils().toJson(detail));
        } catch (JsonProcessingException e) {
